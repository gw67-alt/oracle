...
def detect_sharp_signals_middle_only(curvature_data, threshold=0.8, min_prominence=0.3, mid_range_percent=0.4):
    """
    Detect sharp signals ONLY when they appear in the middle portion of the curvature data.
    ...
    Returns:
        Dictionary with sharp signal information from middle region only
    """
    ...
    mid_start = int(data_length * (0.4 - mid_range_percent/2))
    mid_end = int(data_length * (0.4 + mid_range_percent/2))
...
class OscilloscopeDisplay:
    ...
    def render(self):
        ...
        # Add visual indicator for middle detection zone
        mid_start = int(self.width * 0.3)  # 30% from left
        mid_end = int(self.width * 0.7)    # 70% from left
        self.ax.axvspan(mid_start, mid_end, alpha=0.1, color=self.phosphor_green, label='Detection Zone')
        ...
        self.ax.text(0.02, 0.95, 'CURVATURE OSCILLOSCOPE - MIDDLE DETECTION', transform=self.ax.transAxes, color=self.phosphor_green,
...
# Main execution with middle-only sharp signal detection
...
print("MIDDLE-ONLY Sharp Signal Detection Active")
print(f"Detection zone: middle {MID_RANGE_PERCENT*100}% of oscilloscope trace")
    ...
    for cnt in contours:
        ...
        if area > 50 and circularity > 0.7:
            ...
            if len(cnt) >= 10:
                ...
                middle_signals = detect_sharp_signals_middle_only(
                    processed_curv, 
                ...
                # Print detection results ONLY when signals are in middle
                if middle_signals["count"] > 0:
                    timestamp = time.strftime("%H:%M:%S")
                    print(f"[{timestamp}] Frame {frame_count}: MIDDLE SHARP SIGNALS DETECTED!")
                    print(f"  └─ Detection zone: indices {middle_signals['mid_range'][0]}-{middle_signals['mid_range'][1]} ({middle_signals['mid_range_percent']:.1f}% of graph)")
                    print(f"  └─ Signals found: {middle_signals['count']}")
                    print(f"  └─ Max value: {middle_signals['max_value']:.3f}")
                    print(f"  └─ Avg value: {middle_signals['avg_value']:.3f}")
                    print(f"  └─ Positions: {middle_signals['indices']}")
                    print(f"  └─ Values: {[f'{v:.3f}' for v in middle_signals['values']]}")
                    print("-" * 50)
                ...
                if M["m00"] != 0:
                    ...
                    # Show middle signal indicator
                    if middle_signals["count"] > 0:
                        cv2.putText(display_frame, f'MID-SHARP: {middle_signals["count"]}',
