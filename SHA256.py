...
def detect_quantization(data, threshold_ratio=0.5):
    """
    Determines if the signal shows signs of quantization.
    Quantization is likely if the number of unique data values is small
    compared to the total number of data points.
    """
    if data is None or len(data) == 0:
        return False
    
    num_unique_values = len(np.unique(data))
    # Heuristic: If the number of unique values is less than half the data length,
    # we can suspect quantization. This is a simple but effective test.
    if num_unique_values < len(data) * threshold_ratio:
        return True
    return False

def proof_of_work_function(input_string, difficulty=4):
    """
    Performs a SHA-256 proof-of-work.
    Returns True if the hash starts with a certain number of zeros.
    """
    prefix = '0' * difficulty
    hashed_string = hashlib.sha256(input_string.encode()).hexdigest()
    return hashed_string.startswith(prefix)

def proof_of_work_counter(stop_event):
    """
    Thread function that counts by 1000 and performs proof-of-work.
    """
    print("Starting Proof-of-Work thread...")
    count = 0
    while not stop_event.is_set():
        count += 1
        test_string = "GeorgeW" + str(count)
        result = proof_of_work_function(test_string)
        
        # To avoid flooding the console, we can print less frequently
        # or only when a proof is found.
        if result or count % 500000 == 0: # Print every 500k iterations or on success
             print(f"Count: {count}, String: '{test_string}', Proof Found: {result}")
        
        # This loop runs at full CPU speed. Add a small sleep if you want to reduce usage.
        # time.sleep(0.001)
...
class OscilloscopeDisplay:
    def __init__(self, width=800, height=400, history_length=50):
        ...
        self.is_quantized = False # Variable to hold quantization status
    ...
    def add_waveform(self, data):
        ...
        # Determine if the new data is quantized before adding it
        self.is_quantized = detect_quantization(normalized_data)
    ...
    def render(self):
        ...
        if len(self.history) > 0:
            ...
            self.ax.text(0.02, 0.12, f'RMS:  {rms_val:.3f}', 
                         transform=self.ax.transAxes, color=self.phosphor_green,
                         fontsize=10, family='monospace')
            # Display Quantization status
            quant_status = "YES" if self.is_quantized else "NO"
            self.ax.text(0.80, 0.05, f'QUANTIZATION: {quant_status}', 
                         transform=self.ax.transAxes, color=self.phosphor_green,
                         fontsize=10, family='monospace')
...
if __name__ == "__main__":
    ...
    # Create and start the counter and proof-of-work thread
    pow_thread = threading.Thread(target=proof_of_work_counter, args=(stop_event,))
    pow_thread.start()
        ...
        if contours:
            ...
            if len(largest_contour) >= 10:
                ...
                if len(curv_time_smooth) > 0:
                    safe_curv = np.where(np.abs(curv_time_smooth) > 1e-6, curv_time_smooth, 1e-6)
                    oscilloscope.add_waveform(np.abs(1/safe_curv))
    ...
    pow_thread.join() # Wait for the thread to finish
    cap.release()
